...
- **Route Optimization & ETA**: Tracks route distances and provides real-time ETAs using OSRM routing. ETA minutes included in en_route and arriving customer notifications. Smart Route Planner (`/owner/operations?tab=route-planner`) groups deliveries by time window then postal code area, applies nearest-neighbor distance optimization, shows drive times between stops, and integrates 7-day weather forecasts with severity ratings (good/caution/warning) from Open-Meteo daily forecast API.
