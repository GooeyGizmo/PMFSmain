...
export async function sendWaitlistConfirmationEmail(params: {
  ...
  try {
      ...
      html: `
        ...
        <head>
          ...
          <style>
            ...
            .header h1 { margin: 0; font-size: 24px; }
            .content { padding: 30px; }
            .position-badge { display: inline-block; background: #C67D4A20; color: #C67D4A; padding: 12px 24px; border-radius: 20px; font-weight: 600; font-size: 18px; margin: 15px 0; }
            .footer { background: #f9f9f9; padding: 20px; text-align: center; color: #666; font-size: 14px; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1>Prairie Mobile Fuel Services</h1>
              <p style="margin: 10px 0 0 0; opacity: 0.9;">Waitlist Confirmation</p>
            </div>
            <div class="content">
              <p>Hi ${params.name},</p>
              <p>Thank you for joining our waitlist! We're excited to have you.</p>
              <p>Your position number:</p>
              <center><div class="position-badge">#${params.positionNumber}</div></center>
              <p>We'll notify you as soon as a spot opens up. In the meantime, stay tuned for updates!</p>
            </div>
            <div class="footer">
              <p>Questions? Contact us at ${COMPANY_EMAILS.SUPPORT}</p>
              <p>Prairie Mobile Fuel Services · Calgary, Alberta</p>
            </div>
          </div>
        </body>
    ...
    console.log(`Waitlist confirmation email sent to ${params.email}`);
    return { success: true };
  } catch (error) {
...
export async function sendWaitlistInviteEmail(params: {
  ...
  try {
    ...
    const signupUrl = `${params.baseUrl}/signup?invite=${params.inviteToken}`;

    await client.emails.send({
      from: fromEmail,
      to: params.email,
      subject: `You're Invited! - Prairie Mobile Fuel Services`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          ...
          <style>
            ...
            .header h1 { margin: 0; font-size: 24px; }
            .content { padding: 30px; text-align: center; }
            .cta-button { display: inline-block; background: #C67D4A; color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; margin: 20px 0; font-size: 16px; font-weight: 600; }
            .footer { background: #f9f9f9; padding: 20px; text-align: center; color: #666; font-size: 14px; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hi ${params.name},</p>
              <p>Great news! A spot has opened up and you're invited to create your account with Prairie Mobile Fuel Services.</p>
              <p>Based on your interest in the <strong>${params.interestedTier}</strong> tier, we've reserved a spot just for you.</p>
              <a href="${signupUrl}" class="cta-button">Create My Account</a>
              <p style="color: #666; font-size: 14px; margin-top: 20px;">
                Click the button above to get started with your account setup.
              </p>
            </div>
            <div class="footer">
              <p>Questions? Contact us at ${COMPANY_EMAILS.SUPPORT}</p>
    ...
    console.log(`Waitlist invite email sent to ${params.email}`);
    return { success: true };
  } catch (error) {
